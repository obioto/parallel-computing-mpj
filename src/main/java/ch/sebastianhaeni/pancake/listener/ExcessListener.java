...
public class ExcessListener extends PancakeListener {
    ...
    protected void listen() {
        ...
        Request lengthRequest = MPI.COMM_WORLD.Irecv(lengthBuffer, 0, 1, MPI.INT, getWorker(), Tags.EXCESS_LENGTH.tag());

        if (block(lengthRequest)) {
            return;
        }

        WorkPacket[] work = new WorkPacket[lengthBuffer[0]];
        Request dataRequest = MPI.COMM_WORLD.Irecv(work, 0, lengthBuffer[0], MPI.OBJECT, getWorker(), Tags.EXCESS.tag());

        if (block(dataRequest)) {
            return;
        }
